...
function git(dir: string, args: string[], timeoutMs = 15000): Promise<{ stdout: string; stderr: string }> {
...
export async function gitStatus(dir: string, fetch = false): Promise<GitStatus> {
  ...
  try {
    if (fetch) {
      try {
        await git(dir, ['fetch', '--quiet'], 30000)
      } catch {
    ...
    try {
      ...
      const counts = (await git(dir, ['rev-list', '--left-right', '--count', '@{u}...HEAD'])).stdout
    ...
    const dirty = (await git(dir, ['status', '--porcelain'])).stdout.trim().length > 0
    let remoteUrl: string | null = null
    try {
      remoteUrl = (await git(dir, ['remote', 'get-url', 'origin'])).stdout.trim()
    } catch {
      /* no origin */
    }
...
export async function gitPush(dir: string): Promise<{ ok: boolean; message: string }> {
  try {
    const r = await git(dir, ['push'], 60000)
...
export async function gitPull(dir: string): Promise<{ ok: boolean; message: string }> {
  try {
    const r = await git(dir, ['pull'], 60000)
