...
- **Failure handling:** `dirty` and `conflict` results leave the worktree
  ...
  browser for a web app, otherwise run it) before the next Done. `doFinish` first
  checks `rebaseInProgress` (via `git rev-parse --git-path rebase-merge`), so a
  second Done while a rebase is still paused returns a clear "finish the rebase"
  message instead of tripping the dirty check or restarting the rebase.
